...
iterator begin() {
    ...
    for ( p.m_index = 0; p.m_index<int(this->m_table.size()); p.m_index++ ) {
        if ( !m_table[p.m_index].empty() ) { // loop to find the first non-zero element 
            hash_list_itr q = m_table[ p.m_index ].begin(); 
            p.m_list_itr = q; // attribute to the iterator is set to q --> begining attribute
            return p; // return pointer to beginning 
        }
    }
    p.m_index = -1; // if at end return end value
    return p;
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    std::vector< std::list<KeyType> > old_table = m_table; // copy the existing elements
    m_table.clear();
    m_table.resize( new_size ); // call resize on original table

    for ( unsigned int i=0; i<old_table.size(); ++i ) { // for each elemnt 
        for ( hash_list_itr p = old_table[i].begin(); p!=old_table[i].end(); p++ ) {
            // create the new hash value and push it onto the hash 
