...
enum GRID_STATUS { GRID_CLEAR, GRID_BLOCKED, GRID_GOOD };
...
void print_grid(const std::vector<std::vector<GRID_STATUS> > & blocked_grid,
    ...
    for (unsigned int y=0; y<blocked_grid[0].size(); ++y) {
        ...
        if (x == start_x && y == start_y)
            ...
            std::cout << " X";
        else if (blocked_grid[x][y] == GRID_GOOD) 
            std::cout << " $";
        else 
            std::cout << " .";
        }
...
int recurse2 (std::vector<std::vector<GRID_STATUS> >& blocked_grid, int startx, int starty ) {
    if (blocked_grid[startx][starty] == GRID_BLOCKED) {
        return 0;
    }
    if (startx == 0 && starty == 0) {
        ...
        return recurse2(blocked_grid, startx, starty - 1);
    }
    else if (starty == 0) {
        return recurse2(blocked_grid, startx - 1, starty);
    }
    return recurse2(blocked_grid, startx - 1, starty) + recurse2(blocked_grid, startx, starty- 1);    
}

int recurse3 (std::vector<std::vector<GRID_STATUS> >& blocked_grid, int startx, int starty) {
    if (blocked_grid[startx][starty] == GRID_BLOCKED) {
        return 0;
    }
    if (startx == 0 && starty == 0) {
        ...
        int a = recurse3(blocked_grid, startx, starty - 1);
        if (a == 1) {
            blocked_grid[startx][starty -1] = GRID_GOOD;
            std::cout << "(" << startx << "," << starty - 1 << ")" << std::endl;
        }
        return a;
    }
    else if (starty == 0) {
        int b = recurse3(blocked_grid, startx - 1, starty);
        if (b == 1) {
            blocked_grid[startx - 1][starty] = GRID_GOOD;
            std::cout << "(" << startx - 1 << "," << starty << ")" << std::endl;
        }
        return b;
    }
    int right = recurse3(blocked_grid, startx - 1, starty);  
    if (right > 0) {
        blocked_grid[startx - 1][starty] = GRID_GOOD;
        std::cout << "(" << startx - 1 << "," << starty << ")" << std::endl;
        return 1;
        ...
        int up = recurse3(blocked_grid, startx, starty- 1);  
        if (up == 1) {
            blocked_grid[startx][starty -1] = GRID_GOOD;
            std::cout << "(" << startx << "," << starty - 1 << ")" << std::endl;
            return 1;
...
int main(int argc, char* argv[]) {
    ...
    /*
    int a = recurse2(blocked_grid, start_x, start_y);
    std::cout << a << std::endl;
    */
    int b = recurse3(blocked_grid, start_x, start_y);
    std::cout << b << std::endl;
    print_grid(blocked_grid, start_x, start_y);
    /*
