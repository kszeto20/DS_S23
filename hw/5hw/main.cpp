...
void ShipFreightHelper(MTRand_int32 &mtrand, int num_engines, int num_cars, int min_speed, int max_cars_per_train) {
    ...
    std::cout << "minspeed: " << min_speed << std::endl;
...
void StudentTests() {
  ...
  //
    ...
    PushBack(a, TrainCar::MakePassengerCar());
    PushBack(a, TrainCar::MakeDiningCar());
    PushBack(a, TrainCar::MakePassengerCar());
    PushBack(a, TrainCar::MakeSleepingCar());

    // inspect the cars, the links, the links, and sequential IDs...
    assert (a->isEngine());
    assert (a->prev == NULL);
    assert (a->next->isPassengerCar());
    assert (a->next->prev->isEngine());
    assert (a->next->next->isPassengerCar());
    assert (a->next->next->next->isDiningCar());
    assert (a->next->next->next->next->isPassengerCar());
    assert (a->next->next->next->next->next->isSleepingCar());
    assert (a->next->next->next->next->next->next == NULL);
    assert (a->next->getID() == a->getID()+1);
    assert (a->next->next->getID() == a->next->getID()+1);
    assert (a->next->next->next->getID() == a->next->next->getID()+1);
    assert (a->next->next->next->next->getID() == a->next->next->next->getID()+1);
    assert (a->next->next->next->next->next->getID() == a->next->next->next->next->getID()+1);
